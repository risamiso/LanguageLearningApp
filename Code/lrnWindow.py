...
def createCenterLabels(mainWindow, lines):
    ...
    words = correctingLongWords(words, x, centerLabels)
...
def rndWord():
    ...
    words = correctingLongWords(words, x, globCentLabels)
...
def findingRootWord(words, x, centerLabels, wordToReturn = 0, returnBoolOrWord = "bool"):
    wordsToShow = []

    stepBack = 1
    stepForth = 1
    while "----" in tw.lines[x - stepBack][53:].strip(" \n"):
        stepBack += 1
    if "----" in tw.lines[x][53:].strip(" \n"):
        while "----" in tw.lines[x + stepForth][53:].strip(" \n"):
            stepForth += 1
        stepForth += 1
    ...
    for i in range(stepBack + stepForth + 1):
        if i == 0:
            wordsToShow.append(tw.lines[x - stepBack][:25].strip(" 1234567890.\n-") + " ")
            wordsToShow.append(tw.lines[x - stepBack][25:53].strip(" -") + " ")
            wordsToShow.append(tw.lines[x - stepBack][53:].strip(" \n-") + " ")
        else:
            wordsToShow[0] += (tw.lines[x - stepBack + i][:25].strip(" 1234567890.\n-")) + " "
            wordsToShow[1] += (tw.lines[x - stepBack + i][25:53].strip(" -")) + " "
            wordsToShow[2] += (tw.lines[x - stepBack + i][53:].strip(" \n-")) + " "


    if returnBoolOrWord == "bool":
        if stepBack != 0 or stepForth != 0:
            #liftingLongWords(centerLabels, liftUnits = stepBack + stepForth)
            return True
        else:
            return False

    if returnBoolOrWord == "word":
        return wordsToShow[wordToReturn]


def correctingLongWords(words, x, centerLabels):
    newWords = words.copy()

    if findingRootWord(words, x, centerLabels, returnBoolOrWord = "bool"):
        for i in range(3):
            newWords[i] = findingRootWord(words, x, centerLabels, wordToReturn = i, returnBoolOrWord = "word")
    
    return newWords.copy()