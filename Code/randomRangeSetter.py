...
class TextFrame(tk.Frame):
    def __init__(self, mainWindow, text, **otherStuff):
        ...
        self.label = tk.Label(self, text=text, bg=self["bg"], font = ("Happy Festive", s.scaleSize(30)), fg = "yellow")
        self.label.pack()
...
def createClampButton(mainWindow):
    clampButton = tk.Button(mainWindow, bg = "#722ec9", text = "C", font = ("Happy Festive", s.scaleSize(72)), fg = "yellow", command = lambda: putClamps(mainWindow))
    clampButton.place(x = s.scaleSize(270), y = s.scaleSize(17), width = s.scaleSize(91), height = s.scaleSize(91))
...
def putClamps(mainWindow):
    ...
    if smallVioletSquare is None:
        ...
        if smallVioletSquare is None:
            ...
            smallVioletSquare.place(x = s.scaleSize(631), y = s.scaleSize(83), height = s.scaleSize(20), width = s.scaleSize(20))
        ...
        if clampOne >= thirdWheel.displayStartLine and clampOne <= thirdWheel.displayStartLine + 10:
            clampOneFrame.place(x = 0, y = s.scaleSize(137 + 50*placeForOne), width = s.scaleSize(34), height = s.scaleSize(41))
        if clampTwo >= thirdWheel.displayStartLine and clampTwo <= thirdWheel.displayStartLine + 10:
            clampTwoFrame.place(x = 0, y = s.scaleSize(137 + 50*placeForTwo), width = s.scaleSize(34), height = s.scaleSize(41))
...
def dinamicUpdateClamps():
    ...
    if clampOne >= thirdWheel.displayStartLine and clampOne <= thirdWheel.displayStartLine + 10:
        ...
        clampOneFrame.place(x = s.scaleSize(-100), y = 0)
    if clampTwo >= thirdWheel.displayStartLine and clampTwo <= thirdWheel.displayStartLine + 10:
        clampTwoFrame.place(x = 0, y = s.scaleSize(137 + 50*placeForTwo), width = s.scaleSize(34), height = s.scaleSize(41))
    else:
        clampTwoFrame.place(x = s.scaleSize(-100), y = 0)
